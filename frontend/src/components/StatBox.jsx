...
const StatBox = ({ label, value, icon, trend }) => {
  ...
  const trendLabel = trend > 0 ? 'increased' : trend < 0 ? 'decreased' : 'unchanged';
  ...
  return (
    <div className="bg-white/5 border border-white/10 rounded-lg p-4 flex flex-col items-center justify-center text-center">
      ...
      {hasTrend && (
        <div
          className={`text-xs mt-2 ${trend > 0 ? 'text-green-400' : trend < 0 ? 'text-red-400' : 'text-gray-400'}`}
          aria-label={`${label} ${trendLabel} by ${Math.abs(trend)} percent`}
        >
          {trend > 0 ? '+' : ''}
          {trend}%
        </div>
